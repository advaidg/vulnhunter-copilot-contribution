...
**If delegating to trace subagents (see phase2_hunt.md's dispatch procedure),
include these gates AND the "exhaust all callers" rules in each subagent's
task description.** Findings without recorded gate results are
...
Use your search tool to search for all call sites of the suspect symbol across
the codebase. Restrict to production source file extensions and exclude test
directories.
...
forward trace already performed during hunting. Use your search tool to verify
