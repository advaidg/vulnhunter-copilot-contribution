...
Use your file-search tool (e.g. `codebase`/glob search) to discover all production
source files by language. Issue one search per language extension present — in
parallel if your tool supports concurrent calls, otherwise in quick succession:
...
Use your search tool (text/grep search across the workspace) to find all user-controllable inputs, adapting patterns to
