...
- **Repository** = the `Repository URL` value resolved during the prompt's
  Step 0 (Mandatory First Actions) — normalize SSH origins to https and strip
  any `.git` suffix. Reuse that resolved value; do not re-run the resolution.
