...
The runnable form below is **only usable when the orchestrator prompt says
"terminal tool is AVAILABLE for exploit-test execution"** (i.e. the user chose
Terminal-enabled mode and the `runCommands`/terminal tool is included in this
session). Otherwise produce only the Static Data Flow Trace form further down —
the model has no terminal tool to invoke it and the runnable script is
illustrative only.
