...
> 2. **Small transform you already have everything for** (filter by selected numbers, join two in-context JSONs by key, render a markdown table): the main agent does it directly using its file-write tool. No subprocess, no extra LLM call.
> 3. **Large mechanical scan over a big blob** — delegate to a subagent. Pick the model tier carefully (subagents can be given an explicit model; VS Code's subagent tool caps it at the parent session's cost tier):
...
### Step 1: Fetch issues from GitHub (via your terminal tool)

All network calls happen via your terminal tool, not a Python subprocess — network/proxy/TLS configuration can differ between the two in some environments, and the terminal tool is the one you can fall back to the "`git` + `gh` failure policy" hand-off procedure for if it fails. Discovery and explicit-URL paths both produce the same shape: a JSON array of issue objects with `number`, `url`, `title`, `body`.
...
**(a) Annotate intake with severities + show the table.** Delegate this to a subagent on a lightweight/fast model so the main agent doesn't burn frontier-model tokens iterating over every issue body.

The subagent's job (specify all of this in its task description):
...
5. Overwrite `.vulnhunter-fix/intake.json` with the annotated JSON via its file-write tool. Schema = existing intake fields (number, url, title, body_tampered, markers) + a `severity` field on each issue. **Do NOT copy the `body` field over from raw_issues.json** — body lives in raw_issues.json only; intake.json stays small. Preserve the top-level `owner`, `repo`, `results_dir`, `default_branch` that Step 2 wrote.
...
Why a lightweight model and not the main agent: this is mechanical regex + JSON shuffling over a ~90KB blob. A frontier reasoning model would do the same work for many times the cost. Give the subagent just enough tool access (read + write) to do its job and nothing more.
...
**(c) Ask directly, as a numbered list with comma-separated multi-select.** There's no structured checkbox UI to lean on here — present the options as a numbered list in chat and ask the user to reply with the numbers they want (e.g. `1,3,4` or `all`), adapted to the issue count `N` and whether clustering succeeded:
...
| `2 ≤ N ≤ 4` | — | Numbered list, one line per issue: `"N. #N — VULN-XXX — short title (severity, CWE, location)"`. Ask the user to reply with the numbers they want, comma-separated, or `all`. |
| `N > 4` | **yes** (≥ 2 clusters) | Numbered list over the clusters **in the risk-reduction order** computed in (b): `"N. <cluster name> (N issues) — <rationale + VULN IDs, truncated if long>"`. Up to 4 clusters; if more emerged, merge the smallest related ones or surface them in the 4th slot as `"Other clusters"`. The first cluster (highest risk-reduction score) gets `(Recommended)` appended to its label. |
| `N > 4` | **no** (clustering failed) | Numbered list over severity buckets that appear (Critical / High / Medium / Low, empty buckets omitted): `"N. Critical (3 issues) — <VULN IDs>"`. |

In all `N > 4` cases, also tell the user they can reply with specific issue numbers directly (e.g. `42,55,71`) instead of picking from the list, for fine-grained selection.

**Parsing the answer.** Whatever shape was used, normalize down to a set of issue numbers:

- **Per-issue list selection**: map each chosen list number back to its `#N` issue number from the list you presented.
- **Cluster list selection**: look up each chosen cluster name in `clusters.json` and union its `members[].number` values.
- **Severity buckets** (fallback): take every issue from `intake.json` whose `.severity` matches a selected bucket label.
- **Direct issue numbers**: parse comma-separated `42,43`, `#42, #43`, or single numbers. Reject non-numeric tokens and re-ask.
...
1. Snapshot the full harvest:
   ...
   ```
2. From the user's chat reply, determine the selected issue numbers (per the "Parsing the answer" rules above).
3. Use your file-write tool to overwrite `.vulnhunter-fix/intake.json` with the same JSON shape minus the unselected issue entries. You already have the unfiltered object in context from Step 3(a). Sanity-check that the result still has `.owner`, `.repo`, `.results_dir`, `.default_branch`, and a non-empty `.issues` array.
4. If the user selected `none` (no issues), write an empty `.issues` array, tell the user nothing was done, and exit cleanly.

If the user's reply selects nothing (empty reply, or explicitly says none) treat it as `none` → exit cleanly with a message that nothing was done.
...
**Step 5a: Read the README and produce a draft findings list (frontier-model subagent).**

Delegate this to a subagent on a **frontier reasoning model** — explicitly request one if your subagent tool lets you specify a model. A lightweight/fast model has proven unreliable on the report layout we see in practice — it drops rollup-subsumed findings, misreads multi-CWE rows, and confuses detail-section field labels. A frontier model handles the same input correctly and the cost delta is small compared to the cost of an incorrect findings list propagating through the rest of the pipeline (re-runs, missed findings, manual injection of corrections).

**Important: the subagent has no shell context.** Before dispatching it, the main agent must substitute the actual path of `REPORT_PATH` (read from `.vulnhunter-fix/.report_path`) into the subagent's task description text. Do not pass the literal string `$REPORT_PATH` — the subagent will try to read a file called `$REPORT_PATH/README.md` and fail.

The subagent's job (specify all of this in its task description *with the real absolute path already substituted*):
...
5. Write the result to `.vulnhunter-fix/findings.draft.json` via its file-write tool.
...
If validation fails, re-invoke the Step 5a frontier-model subagent with the validator's stderr appended to the subagent's task description as guidance ("your prior output failed validation: <message> — produce a corrected one"). Do NOT proceed to hashing until validation passes.
