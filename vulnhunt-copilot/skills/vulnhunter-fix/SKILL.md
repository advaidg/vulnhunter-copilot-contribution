...
> **Porting note.** Ported from the Claude Code `vulnhunter-fix` Skill. This
> is the most consequential of the three skills to port — it pushes real
> commits, opens real PRs, and creates real git worktrees, and the upstream
> version has an entire section of hard-won operational discipline around a
> specific tool's (Claude Code's Bash tool) sandbox/TLS failure modes. Real
> changes made, not just terminology:
> - **Task tracking**: `TaskCreate`/`TaskUpdate`/`TaskList` → VS Code
>   Copilot's built-in todo-list tool (`todo_write`/`todo_read`, surfaced in
>   the UI as a todo list) — confirmed to exist and serve the same purpose.
> - **Structured multiple-choice questions**: `AskUserQuestion` has no
>   confirmed Copilot equivalent. Ported as: present the options as a
...
> - **The `git`/`gh` failure-policy section**: rewritten, not just
>   relabeled. VS Code's terminal tool has its *own*, different sandboxing —
>   confirmed to block network access by default with a built-in
>   retry-with-confirmation flow, which is NOT the same failure mode the
>   upstream section was written around (Claude Code's Bash tool had no such
>   retry and needed a full manual hand-off to the user's own terminal every
>   time). This port tries that retry first; the manual hand-off is now a
>   fallback, not the first move. See **`git` + `gh` failure policy** below.
> - **Everything else** — mode dispatch, phase structure, TDD discipline,
>   delivery gates, scripts, references, templates, tests, the Python
>   package — is unchanged. None of it is Claude-specific.

## Reference Files

This skill's workflow reads these files directly. Every prompt file below, in
turn, references the specific scripts/references/templates it needs by
relative path — you do not need a separate top-level link for every file in
this skill's tree; once you're reading a file inside this skill's folder,
your normal file tools can reach anything else under it.

- [prompts/parse.md](./prompts/parse.md) — fork mode, Phase 1
- [prompts/parse_issues.md](./prompts/parse_issues.md) — in-place mode, Phase 1
- [prompts/plan.md](./prompts/plan.md) — in-place mode, Phase 2
- [prompts/plan_fork.md](./prompts/plan_fork.md) — fork mode, Phase 2
- [prompts/implement.md](./prompts/implement.md) — Phase 3 (both modes)
- [prompts/verify.md](./prompts/verify.md) — Phase 4
- [prompts/deliver.md](./prompts/deliver.md) — Phase 5
- [prompts/sweep.md](./prompts/sweep.md)
- [prompts/reviewer_test.md](./prompts/reviewer_test.md)
- [prompts/tier_judgment.md](./prompts/tier_judgment.md)
- [prompts/agent_fix.md](./prompts/agent_fix.md)
- [prompts/worker_agent_common.md](./prompts/worker_agent_common.md)
- [prompts/worker_agent_authz.md](./prompts/worker_agent_authz.md)
- [prompts/worker_agent_config.md](./prompts/worker_agent_config.md)
- [prompts/worker_agent_crypto.md](./prompts/worker_agent_crypto.md)
- [prompts/worker_agent_injection.md](./prompts/worker_agent_injection.md)
- [prompts/worker_agent_resource.md](./prompts/worker_agent_resource.md)
- [config.json](./config.json)
- [collaborators.json](./collaborators.json)
...
Before any other action, decide the mode. Run the canonical dispatcher script and parse its stdout using your terminal tool:
...
The user is inside a GitHub checkout AND supplied fork-mode arguments — both signals are present and only one can be the intended target. **Ask the user directly** to disambiguate — present these two options as a numbered list and wait for their reply:

1. *"In-place — fix `<owner_repo>` (current checkout)"* — discard `$TARGET_REPO`/`$RESULTS_PATH`, re-run dispatch with empty args so the result is `mode=in_place`.
2. *"Fork mode — fix `<target>` using `<results>`"* — `cd` out of the current checkout (or set `OWNER_REPO=""`) and re-run with the original args so the result is `mode=fork`.

Do not guess; the developer's intent is the only signal. Until they answer, do not advance to Step 1.

#### Handling `mode=none`

Neither signal was supplied — not in a git repo and no `TARGET_REPO`/`RESULTS_PATH` arguments. **Ask the user directly** to collect both inputs:

1. *"TARGET_REPO"* — full GitHub URL (e.g., `https://github.com/org/repo`)
2. *"RESULTS_PATH"* — local directory path OR a GitHub URL to a results repo
...
**Step 0: Bind `SKILL_DIR`.**

`SKILL_DIR` is the absolute path of the folder containing this `SKILL.md` —
you can determine it from the path you used to read this file. If for any
reason you can't introspect that, fall back to the default install location,
`~/.copilot/skills/vulnhunter-fix`, but confirm it actually contains
`scripts/detect_mode.sh` before relying on it. Export `SKILL_DIR` (and, for
the shell scripts that read it, `VULNFIX_SKILL_DIR=$SKILL_DIR`) for every
later step in this file and in every prompt file it loads.

**Step 0a: Confirm you're running on a strong reasoning model.** *(in-place / interactive mode only — headless skips this)*
...
# 2. Get the SHA the installed skill is at. The installer (install-copilot.sh)
#    writes this on every install. If the file is missing, the user installed by
...
| `INSTALLED_SHA` empty | Print a single-line warning: `Could not determine installed skill version (no ${SKILL_DIR}/.installed-from). Re-run install-copilot.sh to enable version checks.` — then proceed. |
| SHAs differ | **Stop and tell the user**, using the failure-policy template (⚠️ + bold + fenced block). Suggest the exact re-install command for their checkout: `cd <path-to-cloned-vulnhunter-repo> && git pull origin main && ./install-copilot.sh` — and ask them to re-invoke `/vulnhunter-fix` after the install completes. |
...
```
If any check fails, stop and report the failure to the user. Preflight only validates LOCAL state (tool versions, disk, working-tree-cleanness, etc.) — it does not check network or `gh` auth because subprocess calls launched from a script can't always reliably reach GitHub in every target environment (proxy/cert configuration can differ between a plain subprocess and your own terminal tool).

**Step 1b: Verify `gh` auth via your terminal tool.**
```bash
...
```
This runs through your terminal tool's working context, which may have different network/proxy/TLS configuration than the Python preflight script's subprocess environment — worth verifying independently rather than assuming they match. For in-place mode also verify the user can see `origin` (using the `OWNER_REPO` computed in mode dispatch so the call doesn't depend on `gh repo set-default`):
```bash
...
VS Code's terminal tool sandboxes commands by default on macOS/Linux/Windows, and network access is blocked by default for all domains under that sandbox. When a sandboxed `git`/`gh` call fails on a network operation, **try the built-in retry first**: the terminal tool offers a retry-with-unrestricted-network confirmation before falling back to running the command outside the sandbox — accept that when it's offered, since it resolves the most common cause of failure (the sandbox itself) without any manual hand-off.

If a `git`/`gh` failure persists even after that retry — or presents as a genuine credential/keychain/corporate-proxy problem rather than a plain sandbox network block (TLS certificate errors, keychain interception, VPN-gated hosts) — fall back to the manual hand-off procedure below. This is the same discipline the Claude Code version of this skill needed for its Bash tool, which had no automatic sandbox-retry at all; on Copilot it's a fallback rather than the first move, but the underlying principle — don't thrash through tool substitutions — still holds:
...
When you hit one of these (and the sandbox-retry either isn't offered, doesn't help, or isn't applicable), this is the **only** acceptable sequence:
...
1. **STOP.** Do not retry the same command. Do not retry with different flags. Do not invoke a different but equivalent tool — that is the worst trap. Examples of forbidden "fixes":
   ...
   Every "fix" in this category makes the transcript noisier, wastes the user's time, and never addresses the environmental root cause. The TLS / keychain / sandbox quirks are local to this environment; no command variation will fix them.
...
2. **Tell the user, in plain text, exactly ONE command to run in their own terminal** (NOT a multi-command pipeline, NOT a `cd && cmd` chain — one literal command they can copy and paste). Use this exact format so the command stands out visually instead of getting buried in prose:
   ...
   ````
   ⚠️ `<original tool call>` failed inside the sandbox.
...
This rule applies in every phase — parse, plan, implement, verify, deliver. If you find yourself running a second `git` or `gh` command of the same conceptual kind (clone, fetch, list-issues, …) with no user input in between (beyond one sandbox-retry), you are violating this rule.
...
````
⚠️ `go mod download` failed inside the sandbox (network/proxy block).
...
Read [config.json](./config.json) from this skill's directory for settings (GitHub host, branch prefix, behavior flags).
...
Multi-finding clusters and the interactive collaboration loop both have many steps that the agent will forget if it doesn't externalize them. **Use your todo-list tool (VS Code's built-in todo list — `todo_write`/`todo_read`) from the start of every in-place run and update it aggressively.** Same applies in fork mode for runs with more than ~3 findings.

The granularity:

- **Phase 1 (parse_issues.md)** — one todo item per finding the developer selected. Subject = `<VULN-NNN> — <short title>`. Status starts `pending`. Don't conflate findings into one item even when they're in the same cluster — each finding has its own RED→GREEN evidence to track.
- **Phase 2 (plan.md)** — for each finding, set its todo item's description to the planned approach (test framework, fix strategy, in-cluster sequence position). No status change.
- **Phase 3 (implement.md)** — flip a finding's todo item to `in_progress` when you enter its IP-Step A (exploit demo) and to `completed` only when the commit lands (IP-Step H). If the collaboration loop engages, create **sub-items** for each developer-driven decision (e.g., "Get PoP lib API contract from developer", "Decide between approach A and B", "Add env var to staging config") so the loop's progress is auditable.
- **Phase 4 (verify.md)** — one todo item per cluster, subject = `Verify cluster <name>`. Flip to `completed` only after the full RED→GREEN matrix passes.
- **Phase 5 (deliver.md)** — one todo item per cluster delivery: `Deliver cluster <name>: push, PR, comment source issues`. Flip to `completed` after `gh pr create` returns a URL AND every source issue has been commented with the PR link. **Do not include "close N issues" in the subject** — the skill does not close source `vulnhunter` issues at delivery time; the PR's `Closes #N1, #N2, …` keywords auto-close them on merge (the only safe closure path, since `--mode=verify` keys on issue closure and would reject a fix that was closed before its PR merged).

Re-read your todo list before any handoff (between phases, between findings within a cluster, before entering the collaboration loop, before each commit) so the agent's working set is always reconciled with reality.

The reason: when a cluster has 8 findings and the collaboration loop fires on 2 of them, there are easily 30+ discrete actions in flight. Without externalized tracking, the agent silently drops sub-steps ("Wait, did I run the regression suite for VULN-007?") and either re-runs work or skips it entirely. The todo list makes the dropped steps visible.
...
3. **Ask the user directly**: *"Phase N complete. Proceed to Phase N+1?"* — present `Approve` / `Pause — I want to inspect first` as the two choices and wait for their reply.
4. **Do not read the next phase's prompt file** until the operator answers Approve.

If a phase's expected artifacts are missing or malformed, do NOT ask the approve/pause question. Instead, stop and report what's missing. Do not offer to "proceed anyway."

The checkpoints are not decoration. They exist because prior E2E runs showed the agent skipping the graph substrate, jumping past plan.md, and starting to edit source files without RED evidence. The checkpoints turn silent skips into visible refusals.
...
- **Operator-gated between phases**: this skill runs unattended WITHIN a phase but pauses at every phase boundary for an operator Approve/Pause decision. See "Phase-boundary checkpoints (mandatory)" above.
...
| Script | Purpose |
|--------|---------|
| `scripts/preflight.py` | Verify LOCAL system requirements (Python, git, gh, disk). Auth + network are checked via your terminal tool in SKILL.md Step 1b. |
...
| `scripts/validate_findings_draft.py` | Shape-check `findings.draft.json` output of `parse_issues.md` Step 5a's model extraction pass |
...
Settings in [config.json](./config.json). Key options:
...
The [collaborators.json](./collaborators.json) file controls who gets access to the private fork:
