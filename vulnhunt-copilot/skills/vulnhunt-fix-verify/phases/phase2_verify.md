...
2. Otherwise, search for the recorded `sweep_pattern` across `REPO`.
3. For each match:
   ...
   - If the match is at a location already fixed (search matched but
     reading it shows the safe construct now), that's fine.
4. Outcomes:
   ...
   - **Skipped**: the pattern is too broad to search cleanly (extremely
...
| `INCONCLUSIVE` | The original location no longer exists AND a search can't locate a successor AND no other gate can be answered. Practically: at least three gates `skipped` and `sink_mitigated` not definitive. |
