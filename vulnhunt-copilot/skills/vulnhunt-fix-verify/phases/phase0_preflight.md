...
You have no terminal tool. Use your file-search tool to confirm directory
existence:
...
If `${OUT}` does **not** exist, you cannot create it (no terminal tool, no
`mkdir`). Stop with:
...
Do not write to a path whose parent doesn't exist — your file-write tool
...
have a terminal tool; derive it from the model's clock or — if uncertain —
